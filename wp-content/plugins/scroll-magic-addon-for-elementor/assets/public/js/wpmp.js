    ...
    $(document).ready(function() {
        ...
        $('.scrollMagicControl').each(function(v, i) {
            ...
            if (typeof control.wpmp_pin !== 'undefined') {
                scene.setPin("." + triggerElement, {
                    pushFollowers: typeof control.wpmp_pushfollowers !== 'undefined'
                });
            }
            ...
            if ($(seft).next().hasClass('drawsvg') || $(seft).next().find('.drawsvg').length > 0) {
                ...
                setTweenDraw.fromTo(shapes, {
                    drawSVG: (control.wpmp_draw_from ? control.wpmp_draw_from.value : 0) + "%"
                }, {
                    duration: 1,
                    drawSVG: (control.wpmp_draw_to ? control.wpmp_draw_to.value : 0) + "%",
                    stagger: 1
                });
            ...
            if ($(seft).next().hasClass('splitText') || $(seft).next().find('.splitText').length > 0) {
                ...
                var setSplitText = gsap.timeline(),
                    mySplitText = new SplitText("." + triggerElement, {
                        type: "words,chars"
                    });
